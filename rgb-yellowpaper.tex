...
like in the case of the infamous Lightning network
inbound liquidity \cite{lightningsucks1, lightningsucks4}, routing \cite{lightningsucks2} and 
centralization issues \cite{lightningsucks3, lightningsucks5}\footnotemark.
Also, all layer 2 solutions have state which must be eventually published
...
\footnotetext{The issues are rather specific to the modern
Lightning network based on dual-peer channels \cite{BOLT},
and should be absent in other designs \cite{nucleus, johnlaw1, johnlaw2, johnlaw3, factories}.
Yet, \emph{any} state channels end up with storing the state update in blockchain,
and thus scale as $O(n)$.}
...
However, this early idea was based on practically inconvenient public key tweaking \cite{P2C},
was limited only to a single-asset scenario,
was lacking programmability, support for non-trivial state,
...
Adam Borcany had provided ideas and critics on the zk-STARK compatibility of the protocol and
...
Alan Stepanec, Alex Kravets, Alexandre Poltorak, Alexis Roussel, Alexis Sellier,
Amir Taaki, Andriy Khavryuchenko, Christian Decker, Eric Yang, Hunter Beast,
Jörg (``Yojoe''), Keith Travin, Konstantin Lomashuk, Rene Pickhardt, Vitaly Bulychov.
