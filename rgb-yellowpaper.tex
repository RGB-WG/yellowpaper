...
Collections (variable-size sets of the same-typed objects, both ordered or unordered) are denoted with
...
Instead of following such an approach, RGB is using a different architecture, named SONIC (see Figure \ref{fig:arch}),
...
with \textbf{GFA256} ISA for arithmetic operations with finite field elements (see Table \ref{tab:gfa256}).
It is further extended by \textbf{USONIC} ISA for accessing operation data and memory (see Table \ref{tab:usonic}).
...
AluVM comes with a set of special-purpose control registers, described in the Section \ref{Registers}.
...
The concept of single-use seals was developed as a result of Peter Todd work on cryptographic commitments,
...
which defines destructible state, it must provide a valid witness $w_i+1$
of the single-use seals related to that state being closed
over the $\mathsf{Id}(o_{i_i})$ of the spending operation;
otherwise the operation is considered invalid.
...
When RGB is used on top of Bitcoin \cite{Bitcoin}, or bitcoin-compatible sidechains (such as Liquid, \cite{Liquid}),
...
Briefly, single-use seals are defined as tuples
\noindent
\begin{equation}
...
\begin{description}
    \item[Primary definition] a Bitcoin transaction output, specified either
        as an output index $\xi \in \mathbb{N}_{32}$ inside a seal witness transaction
        for some previously-defined seals (\textsf{wout} variant),
        or as a full outpoint, consisting of transaction id and output number
        $\langle \zeta, \xi\rangle$.
    \item[Secondary definition] either a noise data, concealing 
\end{description}
...
The first bitcoin transaction output, which is either OP\_RETURN or P2TR output,
must contain a deterministic bitcoin commitment to the multi-protocol commitment,
defined in Section \ref{MMB} below.
...
Prime is a layer 1 designed specifically to match the requirements of the client-side validation systems;
which has a much higher potential for scalability comparing to a traditiona blockchain approach \cite{Prime}.
...
Adam Borko had suggested ideas and critics on the zk-STARK compatibility of the protocol and on its operations with Prime,
as well as a failback seal mechanism.
Olga Ukolova had provided a lot of feedback and idea discussions during the protocol design and implementation phase.

This project was supported by donations to LNP/BP Labs from Fulgur Ventures, Hojo Foundation, Pandora Prime SA,
Bitlight Labs, DIBA Inc, iFinex Inc and many individual contributors.
The author is grateful to investors into Pandora Prime SA, as well as Pandora and Ultraviolet projects,
who had allowed the research to be performed when no other funding sources were available.

The author is grateful to his wife, Orlovska Yassia, and family, who were very supportive during all six years
